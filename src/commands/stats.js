...
const { getWeekStart, formatDuration, getTopMember } = require('../utils/statsHelper');
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(opt =>
      opt.setName('user')
        .setDescription('Check a specific member\'s stats (leave blank for server leaderboard)')
        .setRequired(false)
    ),
  ...
  async execute(interaction) {
    ...
    if (target) {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '🎙️ VC Time', value: formatDuration(memberStats.vcSeconds), inline: true },
    ...
    const fmt = (arr, field, formatter = v => String(v)) =>
      arr.map((m, i) => `${['🥇','🥈','🥉','4️⃣','5️⃣'][i]} <@${m.userId}> — ${formatter(m[field] || 0)}`).join('\n') || 'No data';

    const embed = new EmbedBuilder()
      .setTitle('📊 This Week\'s Leaderboard')
      .setColor(0xf39c12)
      .addFields(
        { name: '💬 Top Chatters', value: fmt(top5Msg, 'messageCount', v => `${v} msgs`) },
        { name: '📸 Media Kings', value: fmt(top5Media, 'mediaCount', v => `${v} files`) },
        { name: '🎙️ VC Grinders', value: fmt(top5VC, 'vcSeconds', formatDuration) },
      )
